...
export default function Home() {
  ...
  const getCricketRankings = async () => {
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_API}/auth/getcricketranking`, {
      method: 'GET',
      headers: {
      ...
      .then((response) => {
        ...
        if (response.ok) {
          ...
          setError("Error fetching cricket rankings");
        }
      })
      .catch((error) => {
        console.log(error)
    ...
    <main className='main'>
      ...
      {rankdata && rankdata.ranks && (
        <div className="rankpage">
          <h1>{rankdata.tournament}</h1>
          {rankdata.ranks.map((rank: any, index: number) => (
            <div key={index} className="country-rank">
              <h2>{rank.country}</h2>
              <p>Points: {rank.points}</p>
              <p>Wins: {rank.wins}</p>
              <p>Losses: {rank.losses}</p>
              <p>Total Matches: {rank.total}</p>
              <Image src={rank.flag} alt={`${rank.country} Flag`} width={50} height={50} />
            </div>
          ))}
        </div>
      )}
